...
FONT = ImageFont.truetype(dirname(abspath(__file__)) + "\courier.ttf", size=10)
    ...
    for i in range(0, 601, 7):
        for j in range(0, 601, 7):
            ...
            if int((((b + g + r) // 3) % LEN_DENSITY)) == 0 and pixelIndex != 0:
                draw.text((j, i), DENSITY[pixelIndex-1], font=FONT)
            else:
                draw.text((j, i), DENSITY[pixelIndex], font=FONT)
